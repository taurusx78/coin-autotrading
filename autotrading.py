...
while True:
    try:
        ...
        while not reset and reset_time <= now < reset_time + datetime.timedelta(seconds=30):
            ...
            # 시가가 업데이트 된 경우
            if target != temp or reset_time + datetime.timedelta(seconds=55) <= now:
                target = temp
                reset_time = pyupbit.get_ohlcv(
                    ticker, interval="day", count=1).index[0] + datetime.timedelta(days=1)
                reset = True
        ...
        while (state == 1 or state == 3) and buy_date == 0 and reset_time - datetime.timedelta(days=1) + datetime.timedelta(seconds=1) <= now < reset_time - datetime.timedelta(days=1) + datetime.timedelta(minutes=1):
            now = datetime.datetime.now()
            current_price = pyupbit.get_current_price(ticker)
